...
from networks.unet import UNet, UNet_DS, UNet_URDS
from networks.unet_multitask import UNet_Multitask


def net_factory(net_type="unet", in_chns=1, class_num=3):
    if net_type == "unet":
        net = UNet(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "unet_multitask":
        net = UNet_Multitask(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "unet_ds":
        net = UNet_DS(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "unet_urds":
        net = UNet_URDS(in_chns=in_chns, class_num=class_num).cuda()
