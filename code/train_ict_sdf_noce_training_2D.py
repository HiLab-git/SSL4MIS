...
parser.add_argument('--exp', type=str,
                    default='ACDC/ict_sdf_noce', help='experiment_name')
parser.add_argument('--model', type=str,
                    default='unet_sdf', help='model_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # ICT mix factors
            ict_mix_factors = np.random.beta(args.ict_alpha, args.ict_alpha, size=(args.labeled_bs//2, 1, 1, 1))
            ...
            outputs_tanh, outputs = model(input_volume_batch)
            outputs_soft = torch.softmax(outputs, dim=1)

            with torch.no_grad():
                ema_output_ux0 = torch.softmax(ema_model(unlabeled_volume_batch_0)[1], dim=1)
                ema_output_ux1 = torch.softmax(ema_model(unlabeled_volume_batch_1)[1], dim=1)
                batch_pred_mixed = ema_output_ux0 * (1.0 - ict_mix_factors) + ema_output_ux1 * ict_mix_factors
                gt_dis = util.compute_sdf_multi(label_batch[:].cpu().numpy(), outputs[:args.labeled_bs].shape)
                gt_dis = torch.from_numpy(gt_dis).float().cuda()

            loss_sdf = mse_loss(outputs_tanh[:args.labeled_bs], gt_dis)
            ...
            supervised_loss = loss_dice + args.beta*loss_sdf
            ...
            writer.add_scalar('info/loss_sdf', loss_sdf, iter_num)
            writer.add_scalar('info/consistency_loss', consistency_loss, iter_num)
            writer.add_scalar('info/consistency_weight', consistency_weight, iter_num)

            logging.info(
                'iteration %d : loss : %f, loss_ce: %f, loss_dice: %f, loss_sdf: %f' %
                (iter_num, loss.item(), loss_ce.item(), loss_dice.item(), loss_sdf.item()))
