...
# this doesn't actually get called (could be copy paste from mean teacher)
...
def train(args, snapshot_path):
    ...
    # create two models for cross supervision
    model1 = create_model()
    model2 = create_model()
    ...
    # train method is the built in train() for pytorch models, not the train() function defined here
    # also note that model.train() does not do the training itself, but just sets the model in training mode
    model1.train()
    model2.train()
    ...
    # creating optimizers for both models
    ...
    # SummaryWriter instawnce for tensorboard
    writer = SummaryWriter(snapshot_path + '/log')
    # ouput to console: iterations per epoch = size of minibatch
    ...
    # most of the novel ideas for CPS is encapsulated in this block (cross supervision and definition of loss functions)
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # unsupervised loss calculations, using pseudo labels
            ...
            # combine model1 and model2 losses to get final loss
            loss = model1_loss + model2_loss

            # set gradients to zero
            # because loss.backward does the following for each param x: x.grad += dloss/dx
            # set gradients to zero so the new gradients do not accumulate from previous iterations
