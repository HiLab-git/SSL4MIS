...
parser.add_argument('--exp', type=str,
                    default='ProstateX/DAN', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    DAN_optimizer = optim.Adam(
        DAN.parameters(), lr=1e-4, betas=(0.9, 0.99))
    ...
    for epoch_num in iterator:
        for i, (sampled_batch_labeled, sampled_batch_unlabeled) in enumerate(zip(cycle(trainloader_labeled), trainloader_unlabeled)):
            ...
            DAN_target_unlabeled = torch.tensor(
                [1] * unlabeled_volume_batch.shape[0]).cuda()

            consistency_loss = F.cross_entropy(
                DAN_outputs, DAN_target_unlabeled.long())
            ...
            DAN_outputs = DAN(torch.cat([outputs_soft, outputs_unlabeled_soft], dim=0), torch.cat(
